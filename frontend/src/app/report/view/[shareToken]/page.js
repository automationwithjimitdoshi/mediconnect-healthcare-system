 ...
 *
 * Public report viewer — requires login to see report content.
 * When a patient shares a report link, this page:
 *   1. Checks if the user is logged in (any role)
 *   2. If NOT logged in → shows a branded landing page with "Login to View" CTA
 *      and stores the share token so they land here after login
 *   3. If logged in → fetches and displays the report
 *
 * Create the folder: src/app/report/view/[shareToken]/
 * Place this file as: page.js inside that folder
 */
...
function ParameterRow({ param }) {
  ...
  return (
    <div style={{
      ...
      background: isAbnormal ? bg + '80' : 'white',
    }}>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: isAbnormal ? 700 : 500, color: NAVY }}>{param.name}</div>
        {param.referenceRange && (
          <div style={{ fontSize: 11, color: MUTED }}>Normal: {param.referenceRange} {param.unit}</div>
        )}
      </div>
      <div style={{ textAlign: 'right' }}>
        ...
        {isAbnormal && (
          <div style={{
            ...
            background: bg, padding: '1px 6px', borderRadius: 4, display: 'inline-block',
          }}>
...
function LoginGate({ shareToken, onLogin }) {
  const router = useRouter();

  function goLogin() {
    // Store the share token so after login we redirect back here
    try {
      sessionStorage.setItem('mc_post_login_redirect', `/report/view/${shareToken}`);
    } catch {}
    router.push(`/login?redirect=/report/view/${shareToken}`);
  }
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: `linear-gradient(135deg, ${NAVY} 0%, #1a3a5c 60%, #0e4a3a 100%)`,
      ...
      <div style={{ width: '100%', maxWidth: 420, textAlign: 'center' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: NAVY, marginBottom: 8 }}>
            Medical Report Shared With You
          </div>
          <div style={{ fontSize: 13.5, color: SEC, lineHeight: 1.7, marginBottom: 24 }}>
            Someone has shared a medical report with you via NexMedicon AI.
            Please log in or create a free account to view it securely.
          </div>
          ...
          <div style={{ fontSize: 11.5, color: MUTED, marginTop: 16, lineHeight: 1.6 }}>
            Don't have an account? Register as a patient in under 60 seconds.
            <br />This link expires in 72 hours.
          </div>
        </div>
      </div>
    </div>
...
export default function ReportViewPage() {
  ...
  async function fetchReport(tok) {
    setStatus('loading');
    try {
      // Try the share token endpoint first
      const r = await fetch(`${API}/reports/shared/${shareToken}`, {
      ...
      if (r.status === 404 || r.status === 410) {
        setStatus('error');
        setErrMsg('This report link has expired or is no longer available.');
        return;
      }
      ...
      // Try URL param fallback (client-side generated token)
  ...
  // ── Login gate ──
  if (status === 'gate') {
    return <LoginGate shareToken={shareToken} />;
  }
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', maxWidth: 380 }}>
          ...
          <button onClick={() => router.push('/')} style={{
            padding: '10px 24px', background: BLUE, color: 'white',
  ...
  const patName     = report?.patientName || report?.patient?.firstName
  ...
  return (
    <div style={{ minHeight: '100vh', background: SURFACE, fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>
            NexMedicon AI · Shared Report · {patName}
          </div>
        </div>
        <div style={{
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '20px 16px' }}>
        ...
        {/* Disclaimer */}
        <div style={{
          background: AMBER_P, border: `1px solid #fde68a`, borderRadius: 10,
          padding: '10px 14px', marginBottom: 16, fontSize: 12.5, color: AMBER,
        }}>
        ...
        {findings.filter(f => f.severity !== 'ok').length > 0 && (
          <div style={{ background: 'white', borderRadius: 14, border: `1px solid ${BORDER}`, overflow: 'hidden', marginBottom: 16 }}>
            ...
            {findings.filter(f => f.severity !== 'ok').map((f, i) => (
              <div key={i} style={{
                display: 'flex', gap: 10, padding: '10px 16px', borderBottom: `1px solid ${BORDER}`,
                background: f.severity === 'critical' ? RED_P : f.severity === 'warning' ? AMBER_P : GREEN_P,
              }}>
                <span style={{ fontSize: 18, flexShrink: 0 }}>{f.icon || '•'}</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: f.severity === 'critical' ? RED : f.severity === 'warning' ? AMBER : GREEN }}>
                    {f.title}
                  </div>
        ...
        <div style={{
          background: `linear-gradient(135deg, ${BLUE} 0%, #00796b 100%)`,
          ...
          <div>
            ...
            <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.8)', lineHeight: 1.5 }}>
              Book with a specialist on NexMedicon AI
            </div>
          </div>
          <button onClick={() => router.push('/patient/appointments/book')} style={{
            padding: '10px 22px', background: 'white', color: BLUE,
