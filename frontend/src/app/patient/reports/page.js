 ...
 *  1. Added TopDoctorsCard — shows top 3 Google-rated doctors per specialty
 *     inside the "See a Doctor" tab, one card per recommended specialist.
 *  2. Added ShareReportModal — WhatsApp share button in the results header
 *     and inside the "See a Doctor" tab. Recipient must log in to view report.
 *  3. Added saveSession to auth imports (used by ShareReportModal internally).
 *  4. Added shareOpen / shareRpt state and modal render at bottom of return.
 *  5. Added WhatsApp share button next to "Share with doctor" in the header.
 */
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(12,26,46,0.55)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 420, overflow: 'hidden', boxShadow: '0 12px 40px rgba(0,0,0,0.2)' }}>
        <div style={{ background: NAVY, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'white' }}>Share with Doctor</div>
...
// ── SMS Modal (unchanged from original) ───────────────────────────────────────
function SMSModal({ result, onClose, token }) {
  ...
  async function doSend(e) {
    ...
    try {
      ...
      else { setStatus('error'); setMsg(d.error || 'SMS failed.'); }
...
export default function PatientReportAnalyzer() {
  ...
  // ── NEW: state for WhatsApp share modal ────────────────────────────────────
  const [shareOpen, setShareOpen] = useState(false);
  const [shareRpt, setShareRpt] = useState(null); // { id, type }
  ...
  async function sendReportSMS() {
    ...
    } catch { setSmsResult('❌ Could not reach server.'); }
    setSmsBusy(false);
  }
  ...
  async function startAnalysis(file) {
    ...
    try {
      ...
      if (!r.ok || !d.success) { setError(d.message || 'Analysis failed.'); setPhase('error'); return; }
    ...
    if (phase === 'analyzing') return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: SURFACE }}>
        <div style={{ background: 'white', borderRadius: 16, border: `1px solid ${BORDER}`, padding: 32, maxWidth: 520, width: '100%', textAlign: 'center', boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ fontSize: 12.5, color: MUTED, marginBottom: 22 }}>Language: <strong>{LANGS.find(l => l.code === lang)?.native}</strong> · Usually takes 15–30 seconds</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 24, background: SURFACE }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
          <div>
            ...
            {(result.patientName || result.labName) && (
              <div style={{ display: 'flex', gap: 12, marginTop: 4, flexWrap: 'wrap' }}>
                {result.patientName && <div style={{ fontSize: 12, color: BLUE, background: BLUE_P, borderRadius: 6, padding: '2px 8px', fontWeight: 600 }}>👤 {result.patientName}</div>}
                {result.labName && <div style={{ fontSize: 12, color: TEAL, background: '#e0f2f1', borderRadius: 6, padding: '2px 8px', fontWeight: 600 }}>🏥 {result.labName}</div>}
              </div>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button
              onClick={() => {
                setShareRpt({ id: currentFileId, type: result?.reportType || 'Medical Report' });
                setShareOpen(true);
              }}
              style={{
        ...
        {activeTab === 'plain' && (
          <div style={{ background: 'white', borderRadius: 14, border: `1px solid ${BORDER}`, overflow: 'hidden' }}>
            ...
            {result.parameters?.length > 0 ? result.parameters.map((p, i) => {
              ...
              return (
                <div key={i} style={{ borderBottom: i < result.parameters.length - 1 ? `1px solid ${BORDER}` : 'none', padding: '14px 18px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: 11, color: MUTED }}>{p.unit}</span>
                      <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: chip.bg, color: chip.text, border: `1px solid ${chip.border}` }}>{chip.label}</span>
                    </div>
                  </div>
                  {p.plain && (
                    <div style={{ padding: '9px 12px', background: SURFACE, borderRadius: 8, borderLeft: `3px solid #93c5fd`, fontSize: 13, color: SEC, lineHeight: 1.7 }}>
                      {p.plain}
                    </div>
                  )}
                  {p.referenceRange && (
                    <div style={{ marginTop: 6, fontSize: 11.5, color: MUTED }}>📏 Reference: {p.referenceRange} {p.unit}</div>
                  )}
                </div>
              );
            }) : <div style={{ padding: 40, textAlign: 'center', color: MUTED }}>No parameters found.</div>}
          </div>
        )}
        ...
        {/* ── DOCTORS TAB — updated with TopDoctorsCard + WhatsApp share ── */}
        {activeTab === 'doctors' && (
          <div>
            {(!result.doctors || result.doctors.length === 0) ? (
              <div style={{ background: 'white', borderRadius: 14, border: `1px solid ${BORDER}`, padding: '32px 0', textAlign: 'center', color: MUTED }}>
                <div style={{ fontSize: 32, marginBottom: 10 }}>🏥</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: NAVY, marginBottom: 8 }}>No specific referrals for this report</div>
              ...
              <>
                {result.doctors.map((d, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: 'white', border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 16px', marginBottom: 14 }}>
                      ...
                      {/* ── NEW: Top 3 Google-rated doctors for this specialty ── */}
                      <TopDoctorsCard
                        specialty={d.specialty}
                        token={token()}
                        onBook={(googleDoc) => {
                          router.push(`/patient/appointments/book?search=${encodeURIComponent(googleDoc.name)}`);
                        }}
                      />
                    </div>
                  );
                ...
                {/* Bottom CTA row */}
                <div style={{ padding: '14px 16px', background: BLUE_P, borderRadius: 12, border: `1px solid ${BLUE}20`, marginTop: 4 }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: BLUE, marginBottom: 4 }}>📅 Book an appointment directly</div>
                  <div style={{ fontSize: 12.5, color: '#1e3a5f', marginBottom: 12 }}>Book through NexMedicon AI — your report will be shared automatically.</div>
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <button onClick={() => router.push('/patient/appointments/book')}
                      style={{ flex: 1, padding: '10px', background: BLUE, color: 'white', border: 'none', borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: 'pointer', minWidth: 140 }}>
                      📅 Book appointment
                    </button>
                    {/* ── NEW: WhatsApp share in doctors tab ── */}
                    <button
                      onClick={() => {
                        setShareRpt({ id: currentFileId, type: result?.reportType || 'Medical Report' });
                        setShareOpen(true);
                      ...
                      style={{
                        ...
                        background: '#25d366', color: 'white',
                        fontSize: 13, fontWeight: 700, cursor: 'pointer', minWidth: 140,
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        {/* Share with doctor modal (existing) */}
        {showShare && result && (
              ...
              <div style={{ padding: 20 }}>
                {result?.reportType && (
                  <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 13 }}>
                    ...
                    {result.healthScore != null && <div style={{ color: MUTED }}>Health Score: <strong style={{ color: result.healthScore >= 80 ? GREEN : result.healthScore >= 60 ? AMBER : RED }}>{result.healthScore}/100</strong></div>}
                  </div>
                )}
                {smsResult && (
                  <div style={{ background: smsResult.startsWith('✅') ? GREEN_P : smsResult.startsWith('⚠') ? AMBER_P : RED_P, border: `1px solid ${smsResult.startsWith('✅') ? '#86efac' : smsResult.startsWith('⚠') ? '#fde68a' : '#f5c6cb'}`, borderRadius: 10, padding: '12px 14px', fontSize: 13, color: smsResult.startsWith('✅') ? GREEN : smsResult.startsWith('⚠') ? AMBER : RED, marginBottom: 14 }}>
                    {smsResult}
                  </div>
                )}
                <div style={{ marginBottom: 14 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: MUTED, marginBottom: 6 }}>Phone Number (with country code)</label>
        ...
        {/* ── NEW: WhatsApp Share Report Modal ── */}
        {shareOpen && shareRpt && (
          <ShareReportModal
            reportId={shareRpt.id}
            reportType={shareRpt.type}
            patientName={(() => {
              const u = getUser('PATIENT');
              const p = u?.patient;
              return p ? `${p.firstName || ''} ${p.lastName || ''}`.trim() : (u?.firstName ? `${u.firstName} ${u.lastName || ''}`.trim() : '');
            })()}
            token={token()}
            onClose={() => { setShareOpen(false); setShareRpt(null); }}
          />
        )}
