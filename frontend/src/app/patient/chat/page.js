...
// ── FIXED: Added GREEN_P which was missing and caused ReferenceError in room list
...
function buildDownloadUrl(file, token) {
  ...
  if (file?.id)         return `${API}/files/${file.id}/download`;
  return null;
}
  ...
  if (file.storageUrl || file.fileUrl) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        a.href = url;
        a.download = file.fileName || `file.${ext}`;
  ...
  useEffect(() => {
    ...
    if (tok) {
      fetch(`${API}/chat/rooms?limit=100`, { headers: { Authorization: `Bearer ${tok}` } })
        ...
        .then(d => {
          const n = (d?.data || []).reduce((s, r) => s + (r.unreadCount || 0), 0);
          setChatBadge(n);
        })
        .catch(() => { });
    }
  ...
  useEffect(() => {
    try {
      ...
      const n = u?.patient
        ...
        : (u?.email || 'Patient');
      setName(n);
      setIni(n.split(' ').filter(Boolean).map(w => w[0]).join('').slice(0, 2).toUpperCase() || 'P');
    } catch { }
  }, []);
...
function FileBubble({ msg, isMe, onDelete, onDownload }) {
  ...
  if (msg.isDeleted || msg.content === '[Message deleted]') {
    return (
      <div style={{ padding: '8px 12px', borderRadius: 10, background: SURFACE, border: `1px solid ${BORDER}`, fontSize: 12, color: MUTED, fontStyle: 'italic' }}>
        🗑 File deleted
      </div>
    );
  }
...
export default function PatientChatPage() {
  ...
  // FIXED: All missing refs now declared
  ...
  // FIXED: setLoading(false) is now guaranteed to run in all code paths
    ...
    try {
      ...
      if (list.length === 0) {
        try {
          ...
          const appts = (ad.data || ad.appointments || []).filter(a => ['CONFIRMED'].includes(a.status));
      ...
      console.warn('loadRooms:', e.message);
      setIsDemo(false);
      // FIXED: Show connection error banner when backend is unreachable
      if (e.message.includes('fetch') || e.message.includes('Failed') || e.message.includes('refused')) {
        setConnError(true);
      }
      setLoading(false);
    }
  ...
  async function loadMessages(roomId) {
    ...
    try {
      ...
      if (d.data) {
        setMessages(d.data.map(m => ({
          ...m,
          roomId,
          from: m.senderRole === 'PATIENT' ? 'patient' : 'doctor',
          text: m.content,
          time: fmtT(m.createdAt),
          _file: m.file,
        })));
      }
  ...
  async function capturePatientPhoto() {
    ...
    canvas.toBlob(async blob => {
      ...
      try {
        ...
        fd.append('files', blob, 'photo_' + Date.now() + '.jpg');
  ...
  async function sendMessage(e) {
    ...
    setSending(true);
    try {
      const r = await fetch(`${API}/chat/rooms/${selRoom.id}/messages`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token()}`, 'Content-Type': 'application/json' },
      ...
      const d = await r.json();
      if (d.success && d.data) {
        setMessages(p => [...p, { ...d.data, from: 'patient', text: d.data.content, time: fmtT(d.data.createdAt) }]);
        setInput('');
        if (inputRef.current) inputRef.current.focus();
      } else {
        showToast('❌ ' + (d.message || 'Failed to send'));
      }
    } catch { showToast('❌ Network error'); }
    setSending(false);
  }
  ...
  async function handleFileUpload(e) {
    ...
    try {
      ...
      if (!liveRoom || !['CONFIRMED', 'COMPLETED', 'RESCHEDULED'].includes(liveSt || '')) {
        showToast('🔒 File sharing disabled — appointment is not confirmed.');
        return;
      }
    } catch { }
    ...
    try {
      ...
      fd.append('caption', 'Shared: ' + file.name);
      const r = await fetch(`${API}/chat/rooms/${selRoom.id}/messages/file`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token()}` },
        body: fd,
  ...
  async function deleteMessage(messageId, isDemoMsg) {
    ...
    try {
      const r = await fetch(`${API}/chat/rooms/${selRoom.id}/messages/${messageId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token()}` },
      });
      const d = await r.json();
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'DM Sans, sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* FIXED: Connection error banner */}
        {connError && (
          <div style={{ background: RED_P, borderBottom: '1px solid #f5c6cb', padding: '8px 20px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            <span>⚠️</span>
            <span style={{ fontSize: 12, color: RED, flex: 1 }}>Cannot reach server (port 5000). Please start the backend.</span>
            <button onClick={loadRooms} style={{ padding: '4px 12px', background: RED, color: 'white', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>Retry</button>
          </div>
        )}
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          {rooms.length > 0 && (
            <div style={{ width: 220, background: 'white', borderRight: `1px solid ${BORDER}`, overflowY: 'auto', flexShrink: 0 }}>
              <div style={{ padding: '12px 14px', fontSize: 12, fontWeight: 700, color: MUTED, textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: `1px solid ${BORDER}` }}>
                Conversations
              </div>
              {rooms.map(room => {
                const doc   = room.doctor || room.appointment?.doctor;
                const isSel = selRoom?.id === room.id;
                ...
                return (
                  <div key={room.id}
                    ...
                    style={{ padding: '11px 14px', borderBottom: `1px solid ${BORDER}`, cursor: canSelect ? 'pointer' : 'not-allowed', opacity: canSelect ? 1 : 0.55, background: isSel ? BLUE_P : 'transparent', borderLeft: `3px solid ${isSel ? BLUE : 'transparent'}` }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: NAVY, marginBottom: 2 }}>
                      {doc ? `Dr. ${doc.firstName} ${doc.lastName}` : 'Doctor'}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                      <div style={{ fontSize: 11, color: MUTED }}>{doc?.specialty || ''}</div>
                      {rSt && (
                        <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 99, background: rSt === 'CONFIRMED' ? GREEN_P : rSt === 'COMPLETED' ? BLUE_P : rSt === 'SCHEDULED' ? AMBER_P : '#fdecea', color: rSt === 'CONFIRMED' ? GREEN : rSt === 'COMPLETED' ? BLUE : rSt === 'SCHEDULED' ? AMBER : '#c62828' }}>
                          {rSt === 'SCHEDULED' ? '⏳ Pending' : rSt}
                        </span>
                      )}
                    </div>
                    {!canSelect && rSt === 'SCHEDULED' && (
                      <div style={{ fontSize: 10, color: AMBER, marginTop: 3 }}>Awaiting doctor confirmation</div>
                    )}
                  </div>
                );
              })}
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ background: 'white', borderBottom: `1px solid ${BORDER}`, padding: '11px 20px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: TEAL_P, color: TEAL, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
                {docInit.toUpperCase()}
              </div>
              <div style={{ flex: 1 }}>
              ...
              {!selRoom && (
                <div style={{ background: AMBER_P, border: `1px solid ${AMBER}30`, borderRadius: 8, padding: '5px 10px', fontSize: 11, color: AMBER, fontWeight: 600 }}>
                  No active chat — book appointment first
                </div>
              )}
              <button onClick={() => showToast('Video call launching…')}
                style={{ padding: '6px 14px', border: `1px solid ${BORDER}`, background: 'white', borderRadius: 8, fontSize: 12, cursor: 'pointer' }}>
                🎥 Video Call
              </button>
            </div>
            ...
            <div style={{ background: AMBER_P, borderBottom: `1px solid ${AMBER}30`, padding: '9px 20px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
              <span>📎</span>
              <span style={{ fontSize: 12, color: AMBER, flex: 1 }}>
                {!selRoom ? 'Book an appointment with a doctor to activate chat and share reports.' : "Upload lab reports — they appear in your doctor's Report Review instantly."}
              </span>
              <button onClick={() => router.push('/patient/appointments/book')}
                style={{ padding: '5px 12px', border: `1px solid ${AMBER}60`, background: 'white', borderRadius: 8, fontSize: 11, cursor: 'pointer', color: AMBER, flexShrink: 0 }}>
                Book Appointment
              </button>
            </div>
              ...
              {!loading && messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id || i} style={{ display: 'flex', flexDirection: 'column', maxWidth: '72%', alignSelf: isMe ? 'flex-end' : 'flex-start', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                    {(msg.urgent || msg.isUrgent) && !isMe && (
                      <div style={{ fontSize: 10, color: RED, fontWeight: 700, fontFamily: 'monospace', marginBottom: 3 }}>🚨 IMPORTANT MESSAGE FROM DOCTOR</div>
                    )}
                    {isFile
              ...
              {typing && (
                <div style={{ alignSelf: 'flex-start', display: 'flex', gap: 8, alignItems: 'center' }}>
                  <div style={{ padding: '10px 16px', background: 'white', border: `1px solid ${BORDER}`, borderRadius: 14, display: 'flex', gap: 4, alignItems: 'center' }}>
                    {[0, 0.2, 0.4].map(d => (
                      <div key={d} style={{ width: 6, height: 6, borderRadius: '50%', background: MUTED, animation: `pcBounce 1.2s infinite ${d}s` }} />
                    ))}
                  </div>
            ...
            <div style={{ background: 'white', borderTop: `1px solid ${BORDER}`, padding: '12px 20px', flexShrink: 0 }}>
              {isLocked ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: apptStatus === 'CANCELLED' || apptStatus === 'NO_SHOW' ? RED_P : AMBER_P, borderRadius: 10, border: `1px solid ${apptStatus === 'CANCELLED' || apptStatus === 'NO_SHOW' ? RED + '40' : AMBER + '40'}` }}>
          ...
          <div style={{ width: 260, flexShrink: 0, background: 'white', borderLeft: `1px solid ${BORDER}`, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: 14, borderBottom: `1px solid ${BORDER}` }}>
              <div style={{ background: NAVY, borderRadius: 12, padding: 14, color: 'white' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 9, fontFamily: 'monospace', opacity: 0.5, letterSpacing: '0.1em' }}>AI HEALTH SUMMARY</span>
                </div>
                <div style={{ fontSize: 11, lineHeight: 1.65, opacity: 0.92 }}>
                  Upload your lab reports via the Report Analyzer to build your AI health summary.
                </div>
              </div>
            </div>

            <div style={{ padding: 14, flex: 1 }}>
              <div style={{ fontSize: 10, fontFamily: 'monospace', color: MUTED, letterSpacing: '0.08em', marginBottom: 10 }}>
                  ...
                  <div key={i} style={{ display: 'flex', gap: 8, padding: '8px 0', borderBottom: `1px solid ${BORDER}` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <button onClick={() => handleDownload(file)}
                        style={{ fontSize: 10, color: BLUE, fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 2 }}>
                        ↓ Download
                      </button>
                    </div>
                  </div>
            ...
            <div style={{ padding: 14, borderTop: `1px solid ${BORDER}` }}>
              ...
              {selRoom?.appointment
                ? (
                  <div style={{ background: BLUE_P, borderRadius: 10, padding: '10px 12px', marginBottom: 10 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: BLUE }}>{docName}</div>
                    <div style={{ fontSize: 10, color: MUTED, fontFamily: 'monospace', marginTop: 2 }}>
                      {selRoom.appointment.scheduledAt
                        ? new Date(selRoom.appointment.scheduledAt).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })
                        : 'Scheduled'}
                    </div>
                  </div>
                ) : (
                  <div style={{ fontSize: 12, color: MUTED, marginBottom: 10 }}>No upcoming appointments</div>
                )}
              <button onClick={() => router.push('/patient/appointments/book')}
                style={{ width: '100%', padding: '9px 0', background: BLUE, color: 'white', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                + Book Appointment
              </button>
            </div>
          </div>
      ...
      {camModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 800, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            <button onClick={capturePatientPhoto} style={{ padding: '12px 32px', background: 'white', color: '#0c1a2e', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
              📸 Capture & Send
            </button>
            <button onClick={closePatientCamera} style={{ padding: '12px 24px', background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
              ✕ Cancel
            </button>
          </div>
        </div>
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: 24, right: 24, background: NAVY, color: 'white', padding: '12px 20px', borderRadius: 12, fontSize: 13, zIndex: 9999, boxShadow: '0 4px 20px rgba(0,0,0,0.2)', maxWidth: 400, lineHeight: 1.5 }}>
          {toast}
        </div>
      )}
