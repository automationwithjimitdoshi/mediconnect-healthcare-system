 ...
 *
 * FIXES:
 *  - Uses NEXT_PUBLIC_API_URL env var (works on Vercel + local)
 *  - Fetches from /api/reports/patient/my-files (includes reports uploaded via Report Analyzer)
 *  - Also merges files from /api/files/my (files uploaded directly here)
 *  - Shows date + time of upload on every file card
 *  - Deduplicates by id so no file appears twice
 */
...
export default function PatientFilesPage() {
  ...
  async function loadFiles(t) {
    setLoading(true);
    try {
      // Fetch from BOTH endpoints and merge — covers reports AND direct uploads
      ...
      // Deduplicate by id, keep most recent first
      const seen = new Set();
      const merged = [...list1, ...list2].filter(f => {
  ...
  async function handleUpload(fileList) {
    ...
    if (!allowed.includes(file.type)) {
      showToast('Unsupported file type. Use PDF, JPG, PNG, DOCX.', 'err');
      return;
    }
    if (file.size > 20 * 1024 * 1024) {
      showToast('File too large. Max 20 MB.', 'err');
      return;
    }
    ...
    try {
      ...
      // Upload via files endpoint (no AI analysis — just store the file)
      ...
      await loadFiles(tok()); // Refresh list
    } catch (err) {
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {filtered.map(file => {
                ...
                return (
                  <div key={file.id} style={{ background: 'white', borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 18px', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
                      {/* Analyze button — opens Report Analyzer */}
                      <button
                      ...
                      {fileUrl && (
                        <button
                          ...
                          style={{ padding: '6px 12px', background: SURFACE, color: SEC, border: `1px solid ${BORDER}`, borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                          ↓ Download
                        </button>
                      )}
                    </div>
