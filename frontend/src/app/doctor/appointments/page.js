...
export default function DoctorAppointmentsPage() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'DM Sans, sans-serif' }}>
      <DoctorSidebar active="doctorAppts" />
        ...
        <Modal
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                ...
                <button type="button" onClick={openPharmacyFinder}
                  ...
                  style={{
                    padding: '5px 12px', background: sections.prescription?.trim() ? '#0e7490' : '#94a3b8',
          ...
          {!extracting && (sections.notes || sections.prescription || sections.followUp) ? null : (
            <button onClick={() => autoExtractSections(notes || Object.values(sections).join('\n'), modal.appt.id)}
              ...
              style={{
                marginTop: 10, width: '100%', padding: '7px', border: `1px dashed ${BLUE}40`, borderRadius: 8,
      ...
      {pharmacyModal && (
        <div onClick={e => { if (e.target === e.currentTarget) setPharmacyModal(false); }}
          style={{
            position: 'fixed', inset: 0, background: 'rgba(12,26,46,0.65)', zIndex: 600,
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {parsedMeds.map(med => (
                    <button key={med} onClick={() => setSelectedMed(med)}
                      style={{
                        padding: '4px 12px', borderRadius: 99, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', border: '1.5px solid',
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
              ...
              {!pharmaLoading && pharmacies.length > 0 && (
                <>
                  ...
                  {pharmacies.map((p, i) => (
                    <div key={p.id} style={{
                      border: `1px solid ${BORDER}`, borderRadius: 12, padding: '12px 14px', marginBottom: 8,
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
                          ...
                          {p.mapsUrl && (
                            <a href={p.mapsUrl} target="_blank" rel="noopener noreferrer"
                              style={{
                                fontSize: 11.5, color: BLUE, fontWeight: 600, textDecoration: 'none',
